...
./vendor/bin/phpstan analyse --configuration=phpstan-ci.neon --no-progress --no-interaction --error-format=checkstyle | reviewdog -name="phpstan" -f=checkstyle -reporter="${INPUT_REPORTER}" -level="${INPUT_LEVEL}" && \
